...
> [!TIP]
>
> If the default `8000` port is taken application will select random port automatically, see console output to learn which. You can change the default using `kfe --port <other port>`. For more options run `kfe --help`.
>
> If you see CUDA out of memory errors you can still run the application using CPU with `kfe --cpu`. The transcription model is the most resource demanding, see the next section for instruction how to change it. If you enabled LLM descriptions and notice errors try again with disabled.
...
- Vision LLM - (optional) for each image and video (from which single image frame is extracted) application generates few sentences of natural-language description using [deepseek-ai/Janus-Pro-1B](https://huggingface.co/deepseek-ai/Janus-Pro-1B) model.
- Text embeddings - application generates embeddings of each type of text that can be searched (descriptions that you can write manually, transcriptions, OCR results, LLM descriptions) using [BAAI/bge-large-en-v1.5](https://huggingface.co/BAAI/bge-large-en-v1.5) model.
...
Vision LLM requires similar amounts of resources as the large transcription model, there is no lighter-weight alternative (though you may try searching some quantized versions). During directory selection you may opt out of using this model. Application will work fine without it. To change prompt used with this model see [backend/kfe/features/vision_lm_engine.py](backend/kfe/features/vision_lm_engine.py).
...
3. For each file, application extracts relevant text (OCR for images, transcriptions for videos and audio) and lemmatizes it. Results are written to the database so it can be done only once. If you toggled generating LLM descriptions when adding the directory those descriptions will also be generated and processed in the same manner as other types of text.
...
print(embeddings.__dict__.keys()) # ['description', 'ocr_text', 'transcription_text', 'clip_image', 'clip_video', 'llm_text']
print(embeddings.transcription_text.embedding.shape) # (1024, )
```

To see all endpoints open http://localhost:8000/docs. To perform a search you can run:

```sh
curl -X POST "http://localhost:8000/files/search?offset=0&limit=10" \
